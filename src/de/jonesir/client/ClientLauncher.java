...
public class ClientLauncher {
	...
	private static final boolean dataIsEncoded = false; // decide whether to be tranfered data is encoded or not
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < ClientLauncher.blockCount; i++) {
			...
			if (!ClientLauncher.dataIsEncoded) {
				...
				if ((i + 1) % 4 == 0) {
					System.out.println("packetUnitIndex=" + packetUnitIndex);
					String[] encodedPacket = Encoder.encode_apache(packetUnit);
					System.out.println("ecodedPacket.size()="+encodedPacket.length);
					packetUnitIndex = 0;
